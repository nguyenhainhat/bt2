...
const body = select('#head-page'),
	...
	navbarTop = select('.navbar .navbar-top'),
	navContainer = select('.navbar .container');
...
// Caption
const headerSlider = selectAll('.header-slider_img h2');
const textSlider = selectAll('.header-slider_img p');
const buttonSlider = selectAll('.header-slider_img a');
...
// init Isotope
var $filterItem = $('#portfolios').isotope({
...
$('.portfolio-list').each(function (i, buttonGroup) {
	...
	$buttonGroup.on('click', '.portfolio-list_items', function () {
		$buttonGroup
			.find('.portfolio-list_active')
			.removeClass('portfolio-list_active');
...
function resizeFunction() {
	...
	owlCarouselItem.forEach((eml) => {
		...
		owlCarousel.style.width =
			widthOwlContainer * owlCarouselItem.length + 24 + 'px';
	});
...
owlCarousel.style.transform =
	'translateX(' + (-100 * counter) / owlCarouselItem.length + '%)';
...
// Scroll active link
const sections = selectAll('.section-padding');
...
function scrollActive() {
	...
	sections.forEach((current) => {
		...
		if (scrollY > sectionTop && scrollY <= sectionHeight + sectionTop) {
			document
				.querySelector('.navbar-collapse li a[href="#' + currentId + '"]')
